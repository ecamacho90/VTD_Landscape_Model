function [dydt,ChirTime]=VTD_Landscape_Model_Pred_exp(t,y,p, dt, ChirTime)
...
% if p(1)<=0.5 && ChirTime>0 %If Chir off put ChirTime negative and reduce
%     ChirTime = -ChirTime-(2*p(1)-1)*dt;
%     ChirFactor = ((-ChirTime/p(23))^3)/(p(24)+(-ChirTime/p(23))^3);
%     p(1)= min(ChirFactor+p(1),1);
% elseif p(1)<=0.5 && ChirTime<0 %If Chir off and ChirTime negative reduce
%     ChirTime = ChirTime-(2*p(1)-1)*dt;
%     ChirFactor = ((-ChirTime/p(23))^3)/(p(24)+(-ChirTime/p(23))^3);
%     p(1)= min(ChirFactor+p(1),1);
%     if ChirTime>-0.01
%         ChirTime=0;
%     end
% elseif p(1)>0.5 && ChirTime>=0%If Chir on increase ChirTime
%     ChirFactor = ((ChirTime/p(23))^3)/(p(24)+(ChirTime/p(23))^3); %Parameters lambda and mu (2 parameters)
%     ChirTime = ChirTime+(2*p(1)-1)*dt; %Update Chir Time
%     p(1)=min(ChirFactor+ p(1),1);
% elseif p(1)>0.5 && ChirTime<0%If Chir on increase ChirTime
%     ChirTime = -ChirTime+(2*p(1)-1)*dt;
%     ChirFactor = ((ChirTime/p(23))^3)/(p(24)+(ChirTime/p(23))^3); %Parameters lambda and mu (2 parameters)
%     ChirTime = ChirTime+p(1)*dt;
% end
...
mapped(6:11) = [0.08   8    0.08    -3    -0.7        p(25)]*(1-p(1))+...
               [0.08   8    0.08    -3    -0.7        p(26)]*(p(1));   %2 parameters          
fact = mapped(6)*(y(1,:)+mapped(7)).^2+mapped(8)*(y(2,:)+mapped(9)).^2+mapped(10);

lowbound=find(fact<1.e-10);
if lowbound
    fact(lowbound)=1.e-10*ones(size(lowbound));
end
