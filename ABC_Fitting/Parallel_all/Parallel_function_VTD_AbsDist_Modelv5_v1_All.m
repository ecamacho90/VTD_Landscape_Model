...
%% For loop
function Parallel_function_VTD_AbsDist_Modelv5_v1_All()
%%
...
Numberofparticleseachjob = 1;%Nmax/jobmax;
...
ParticlesMatrixaux = zeros(ParticlesMatrixaux);
FatesMatrixaux = zeros(FatesMatrixaux);
ErrorParamsaux = [];


% Loop to call jobs
parfor partnum = 1:Nmax
    
    [NewData,NewFates,errorparams]=ABC_SMC_Algorithm_OLCM_VTD_landscape_v1_Gauss_AbsDistance_All(mutantstofit,parfitnumbers,EpT,T,Tmax,Nmax,Numberofparticleseachjob,PreviousData,namenewdata,CovarianceMatricesParticles,NoClassifiedOption,weightsdistances,streamnum,partnum);
    ParticlesMatrixaux(partnum,:) = NewData;
    ErrorParamsaux = [ErrorParamsaux;errorparams];
    FatesMatrixaux(:,:,:,partnum) = NewFates;                                                        
...
NextEpt = quantile(squeeze(ParticlesMatrixaux(:,35)),0.03);
%save(namenewdata)
save([pathtodata,'/',namenewdata])
