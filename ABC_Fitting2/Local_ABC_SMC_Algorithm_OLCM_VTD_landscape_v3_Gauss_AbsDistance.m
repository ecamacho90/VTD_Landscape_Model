function Local_ABC_SMC_Algorithm_OLCM_VTD_landscape_v3_Gauss_AbsDistance(mutantstofit,parfitnumbers,EpT,T,Tmax,Nmax,Numberofparticleseachjob,PreviousData,namenewdata,CovarianceMatricesParticles,NoClassifiedOption,weightsdistances,streamnum,jobnum)
...
%     lambda,mu,e1,e2,alpha, gamma, psi, epsilon, d0, d1, d2, d3, D ];
...
%Initial parameters:
initpar = [0,   1,   -9, 4.5, -3,  -20, 7 , -8, -7, -3, 8, 10 ,  -8, -3, -3,  -18, -12 ,  -6, -13, -3,  -17, -35, 24, 1, 0.03, 0.03, -8, 3, 2, 2, -1,0.5,0.5,0.5,0.2];
...
if NoClassifiedOption
    %load('/cluster/elenameritxelldata/DataProp180814_ClusteredDataWithThd_v3.mat','DataToFit')
    load('DataProp180814_ClusteredDataWithThd_v3.mat','DataToFit')

%Clustering parameters
    NumDims = 2;
    NumClust = 7;
    Initial.mu = [-10 5; -7 0; -3 3; 3 -3; 3 3; -3 -3; 0 0];
...
%save(['/cluster/elenameritxelldata/paramFatesM5v3_',num2str(jobnum)])
...
%        paramaux = initpar;
       ...
       % priorOK = 1;
        ...
        while (parami<=numel(parfitnumbersi))
            ...
            if priorOK && rem(parami,4)==0 %Modify if fitting c
...
%                 subparamaux
       ...
       % if priorOK
        if isempty(sublandscapefit)   
            CheckLandscapesResult = CheckLandscapesResult+1;
            y0 = [paramaux(27)-sqrt(-paramaux(31)), paramaux(28)]';
